...
def base_model():
    ...
    def train_ARIMA(status):
        ...
        forecast_mean_df = pd.DataFrame({"predicted_mean":forecast_values.values})
        forecast_mean_df.to_csv("~/ML_spark/dag_spark/Causal_Alpha/dataset/base_models/forecast_values.csv")
        return "forcast values generated"
 
    @task()
    def train_LSTM(forcast_status):
        # Create sequences
        data = pd.read_csv('~/ML_spark/dag_spark/Causal_Alpha/dataset/base_models/nifty50.csv', index_col=0)
        forecast_values = pd.read_csv("~/ML_spark/dag_spark/Causal_Alpha/dataset/base_models/forecast_values.csv")
        ...
        final_forecast_df = pd.DataFrame({"Date": future_dates, "Actual_Close": y_last_100.flatten(),
                                        ...
                                        "Predicted_Close_ARIMA": forecast_values["predicted_mean"].values})
        ...
        print(len(y_last_100.flatten()))  
        print(len(future_predictions_inv.flatten()))  
    ...
    forecast_value = train_ARIMA(training_data)
